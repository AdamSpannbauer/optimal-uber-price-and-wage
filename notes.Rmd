...
check_fixed_payout_constraint <- function(k, K, lambda, lambda_bar, c, d, mu, alpha, tolerance = 0.01) {
  v1 <- k^2
...
profit_function <- function(k, K, lambda, lambda_bar, c, d, mu, alpha) {
  ...
  price <- price_rate(lambda, lambda_bar, c, d, Wq)
  wage <- wage_rate(k, K, lambda, d)
...
display_results <- function(res) {
  ...
  display_res |>
    ...
    mutate(Value = as.numeric(sprintf("%.2f", Value)))
}
```
...
There Table 1 seems to prefer a solution with higher price and lower wait times.
...
find_optimal_k_and_lambda <- function(
  ...
  possible_k <- 1:K
  possible_lambda <- seq(lambda_tol, k, by = lambda_tol)
  ...
  for (k in 1:K) {
    for (lambda in possible_lambda) {
      ...
      if (res$meets_rho_constraint & res$meets_fixed_payout_constraint) {
        ...
        if (isTRUE(all.equal(res$profit, best_outcome$profit, tolerance = 0.001))) {
